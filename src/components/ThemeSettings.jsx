...
const ThemeSettings = () => {
  ...
  return (
    <div className='w-screen bg-half-transparent fixed nav-item top-0 right-0'>
      <div className='float-right w-400 h-screen dark:text-gray-200 bg-white dark:[#484852]'>
            ...
            {themeColors.map((item, index) => (
              <TooltipComponent key={index} content={item.name} position='TopCenter'>
                <div className='relative mt-2 cursor-pointer flex gap-5'>
                  <button  type='button' className='w-10 h-10 rounded-full cursor-pointer' style={{background: item.color}} onClick={() => {setColor(item.color)}}>
